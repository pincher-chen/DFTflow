from calculation.vasp.analysis import MatEnergy, MatGenDos, MatGenStruct, MatGenBandStructureSymmLine
...
if __name__ == "__main__":
  ...
  root = sys.argv[1]+"/Scf"
  hmat_id = sys.argv[1].split('/')[-1]
  data = MatGenStruct(root).get_property()
  data["hmat_id"] = hmat_id
  #print(data)
  # optimized_cif
  try:
    data2 = MatGenStruct(root).get_cif()
    #print(type(data2),data2.keys())
    data['cif_data'] = data2['cif_data']
  except:
    data['cif_data'] = ""
  # energy
  try:
    data3 = MatEnergy(root, -1).get_results()
    #print(type(data3),data3.keys())
    data['energy'] = data3['energy']
    data['formation_energy'] = data3['formation_energy']
  except:
      data['energy'] = 10000
      data['formation_energy'] = 10000
  # band 
  try:
    root = sys.argv[1]+"/Band"
    data4 = MatGenBandStructureSymmLine(root).get_matgen_results()
    #print(type(data4),data4.keys())
    data['Band_Structure'] = data4['Band_Structure']  
  except:
    data['Band_Structure'] = []
  # dos
  try:
    root = sys.argv[1]+"/Dos"
    data5 = MatGenDos(root).get_matgen_results()
    #print(type(data5),data5.keys())
    data['Density_of_states'] = data5['Density_of_states']
  except:
      data['Density_of_states'] = [] 
  #client=pymongo.MongoClient(host="12.11.70.140",port=10102)
  #db=client["vir_data"]
  #collection=db["sp2"]
  #collection.update_one({"hmat_id":hmat_id},{"$set":data},upsert=True)

  with open(sys.argv[1] + "/result.json",'a') as f:
    json.dump(data,f)
  # magx 
  #root = sys.argv[1]+"/mag"
  #data = MatGenStruct(root).get_mag()
